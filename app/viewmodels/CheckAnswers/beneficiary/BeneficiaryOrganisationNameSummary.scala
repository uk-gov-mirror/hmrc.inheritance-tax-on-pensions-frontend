...
object BeneficiaryOrganisationNameSummary {
  ...
  def row(srn: Srn, index: Int, answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(BeneficiaryOrganisationDetailsPage(index)).map { details =>
      SummaryListRowViewModel(
        key = "beneficiaryOrganisationDetails.name.checkYourAnswersLabel",
        value = ValueViewModel(details.beneficiaryTrstName),
        actions = Seq(
          ActionItemViewModel(
            "site.change",
            routes.BeneficiaryOrganisationDetailsController.onPageLoad(srn, index, CheckMode).url
          ).withVisuallyHiddenText(messages("beneficiaryOrganisationDetails.name.change.hidden"))
        )
      )
    }
}
