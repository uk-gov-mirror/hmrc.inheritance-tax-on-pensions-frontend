...
class PrOrganisationCountrySummarySpec extends SpecBase {
  ...
  "PrOrganisationCountrySummary" - {
    ...
    "must return None when data is not present" in {
      PrOrganisationCountrySummary.row(srn, emptyUserAnswers) mustBe None
    }
    ...
    "must show the country name and link to the address lookup journey" in {
      ...
      val result =
        PrOrganisationCountrySummary.row(srn, userAnswers, _ => "United Kingdom").value

      result.key.content mustBe Text(messages("prOrganisationCountry.checkYourAnswersLabel"))
